				      ...
				      bool force_reclaim)
{
	...
	bool skip_reference_check = false;  /* <--- Add this exact line here */
	int pgactivate = 0;
...
static int should_skip_vma(unsigned long start, unsigned long end, struct mm_walk *walk)
{
	...
	struct lru_gen_mm_walk *priv = (struct lru_gen_mm_walk *)walk->private;
	struct address_space *mapping; /* <--- Add this exact line right here */
			 ...
			 unsigned long *start, unsigned long *end)
{
	...
	/* If our tracked VMA is empty, synchronize it with the engine's current VMA */
	if (!vma)
		return false;
	...
	while (vma) {
		...
		if ((next & mask) != (vma->vm_start & mask))
			return false;

		if (should_skip_vma(vma->vm_start, vma->vm_end, walk)) {
			vma = vma->vm_next;
			continue;
		}

		*start = max(next, vma->vm_start);
		next = (next | ~mask) + 1;
		/* rounded-up boundaries can wrap to 0 */
		*end = next && next < vma->vm_end ? next : vma->vm_end;
...
static void walk_mm(struct lruvec *lruvec, struct mm_struct *mm, struct lru_gen_mm_walk *walk)
{
	...
	/* 1. Define the operations structure using 4.19 names */
	struct mm_walk_ops walk_ops = {
		.test_walk = should_skip_vma,
		.pud_entry = (void *)walk_pud_range, /* Map p4d logic down to pud */
	};
	...
	do {
		...
		/* 4.19 uses mmap_sem instead of modern mmap_lock */
		if (down_read_trylock(&mm->mmap_sem)) {
			unsigned long start = walk->next_addr;
			unsigned long end = mm->highest_vm_end;

			/* 2. Call 4.19 signature: walk_page_range(mm, start, end, ops, private) */
			err = walk_page_range(mm, start, end, &walk_ops, walk);
